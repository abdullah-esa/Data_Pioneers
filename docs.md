...
**Project Team:** Data Pioneers (DEPI R4)[span_0](start_span)[span_0](end_span)  
**Contributors:** Abdallah Ahmed, Yousef Elhwehy, Basel Mohamed, Moaz Elkersh[span_1](start_span)[span_1](end_span)  
**Tech Stack:** PostgreSQL, Power BI, VS Code DB Client[span_2](start_span)[span_2](end_span)[span_3](start_span)[span_3](end_span)
...
Every year, thousands of lives are severely impacted by road traffic accidents across the United Kingdom[span_4](start_span)[span_4](end_span). Behind the sprawling tabular registries maintained by emergency services lies an intricate web of hidden patterns, behavioral correlations, and infrastructure systemic failures[span_5](start_span)[span_5](end_span). 

This project addresses the operational challenge faced by public safety authorities and urban planning committees: transforming historical, flat-file accident logs into high-throughput relational data architectures[span_6](start_span)[span_6](end_span)[span_7](start_span)[span_7](end_span). By establishing an enterprise-grade analytics infrastructure, this system uncovers critical environmental and temporal risk factors[span_8](start_span)[span_8](end_span). The final system serves as a scalable diagnostic tool designed to optimize law enforcement deployment, evaluate infrastructure modifications, and ultimately implement data-driven policies that save lives[span_9](start_span)[span_9](end_span)[span_10](start_span)[span_10](end_span).

### Objectives & Scope
*   **Database Normalization & Optimization:** Refactor flat-file legacy source datasets into a highly indexed relational database to mitigate update anomalies and optimize query speeds[span_11](start_span)[span_11](end_span).
*   **Data Quality Assurance:** Enforce strict technical type casting, primary/foreign key relationships, and data-imputation techniques directly within the database layer to achieve clean datasets[span_12](start_span)[span_12](end_span).
*   **Enterprise Dimensional Modeling:** Design and implement a multi-fact Star Schema that maximizes business intelligence indexing and cross-filtering efficiency[span_13](start_span)[span_13](end_span)[span_14](start_span)[span_14](end_span).
*   **Interactive BI Deployment:** Construct a high-fidelity, multi-page executive dashboard detailing micro-level infrastructure correlations and macro-level spatial/temporal trends[span_15](start_span)[span_15](end_span).

#### Out of Scope
*   Real-time streaming ingestion pipelines (historical data analysis focus)[span_16](start_span)[span_16](end_span).
*   Predictive machine learning models for real-time accident forecasting.
*   Automated alerts or notifications for emergency responses.
...
+---------------+      +-----+--------------+          +-----------------------+
| PK | light_id |      | PK  | Accident_Index|         | PK | Accident_Index   |
 ...
 * Color Palette Philosophy (Corporate Clean High-Contrast):
   ...
   * Accent Metric Identifier: Electric Blue #1E90FF acts as the uniform focus color. It is applied exclusively to trend indicators, active charts, and high-priority metrics to ensure direct focal alignment.
